...
class TestMiyaTerminalAI:
    ...
    async def test_process_with_ai_enabled_success(self, ai_instance):
        ...
        with patch.object(ai_instance, '_analyze_ai_response', 
                         return_value=expected_result) as mock_analyze:
            result = await ai_instance.process_with_ai("测试输入")
    ...
    def test_extract_commands_from_response(self, ai_instance):
        ...
        test_responses = [
            ("执行 ls -la 命令", ["ls -la"]),
            ("运行 git status 和 git log", ["git status", "git log"]),
            ("执行以下命令：npm install && npm start", ["npm install && npm start"]),
            ("请执行: python main.py --help", ["python main.py --help"]),
        ]
        ...
        for response, expected_commands in test_responses:
            ...
            for expected in expected_commands:
                found = any(expected in cmd.command for cmd in commands)
                assert found, f"未找到命令: {expected}"
...
class TestMiyaMultiTerminalShell:
    ...
    def shell_instance(self):
        ...
        mock_ai.get_prompt_info = MagicMock(return_value={
            'memory_enabled': True,
            'memory_max_count': 10
        })
    ...
    async def test_get_prompt_input_no_active_terminal(self, shell_instance):
        ...
        # 模拟输入
        test_input = "测试输入"
        with patch('run.multi_terminal_main_v2.chinese_input', 
    ...
    async def test_execute_ai_commands(self, shell_instance):
        ...
        # 验证每个命令都被执行
        assert shell_instance.orchestrator.terminal_manager.execute_command.call_count == 2
    ...
    async def test_run_shell_exit(self, shell_instance):
        """测试运行Shell - 退出"""
        # 模拟用户输入"exit"
        with patch.object(shell_instance, '_get_prompt_input', 
                         AsyncMock(side_effect=["exit"])):
            with patch.object(shell_instance, '_process_input', 
                             AsyncMock()):
                shell_instance.running = True
    ...
    def test_is_system_command(self):
        """测试系统命令检测"""
        shell = MiyaMultiTerminalShell()
        
        # 系统命令
        assert shell._is_system_command("exit") == True
        assert shell._is_system_command("quit") == True
        assert shell._is_system_command("help") == True
        assert shell._is_system_command("clear") == True
        assert shell._is_system_command("create") == True
        
        # 非系统命令
        assert shell._is_system_command("ls") == False
        assert shell._is_system_command("test") == False
        assert shell._is_system_command("") == False
