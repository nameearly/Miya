            ...
            try:
                ...
                try:
                        ...
                        if config_path.exists():
                            ...
                            for model_id in model_ids_to_try:
                                ...
                                try:
                                    ...
                                    logger.info(f"[终端聊天] AI客户端初始化成功: {model_id}")
                                    break
                                except Exception as client_error:
                                    logger.debug(f"[终端聊天] 尝试模型 {model_id} 失败: {client_error}")
                                    continue
                    ...
                    try:
                        ...
                        logger.info(f"[终端聊天] 工具系统初始化成功，已加载 {len(tool_subnet.registry.tools)} 个工具")
                    except Exception as e:
                    ...
                    if tool_subnet:
                        tools_info = []
                        for tool_name, tool in list(tool_subnet.registry.tools.items())[:10]:  # 只显示前10个工具
                            try:
                                tool_config = tool.config if hasattr(tool, 'config') else {}
                    ...
                    if memory_engine:
                        try:
                            ...
                            if memory_context:
                                messages.append(AIMessage(role="system", content=f"记忆上下文:\n{memory_context}"))
                        except Exception as e:
                            ...
                            for tool_name, tool in tool_subnet.registry.tools.items():
                                try:
                                    ...
                                    if tool_display_name in message.lower() or tool_name.lower() in message.lower():
                                        ...
                                        logger.info(f"[终端聊天] 工具执行结果: {result[:100]}...")
                                        break
                                except Exception as tool_error:
                                    logger.error(f"[终端聊天] 工具执行失败: {tool_error}")
                        except Exception as e:
                    ...
                    if model_client:
                        # 如果有工具调用结果，将其作为上下文
                        if tool_call_result:
                            messages.append(AIMessage(role="system", content=f"工具执行结果:\n{tool_call_result}"))
