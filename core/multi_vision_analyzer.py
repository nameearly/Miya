...
class MultiVisionAnalyzer:
    ...
    def __init__(self):
        ...
        self.keyword_mapping = {
            "人物": ["人", "人脸", "肖像", "表情", "动作", "人物", "面部", "微笑", "眼神"],
            "自然": ["风景", "山水", "树木", "花草", "天空", "海洋", "自然", "山脉", "河流"],
            "建筑": ["建筑", "房屋", "大楼", "桥梁", "道路", "建筑", "室内", "室外", "城市"],
            "动物": ["动物", "宠物", "鸟类", "鱼类", "昆虫", "动物", "猫", "狗", "鸟"],
            "文字": ["文字", "文本", "文字内容", "文档", "截图", "文字", "代码", "印刷"],
        ...
        async with self._initialization_lock:
            ...
            for model_id, config in self.models.items():
                ...
                if config.model_type == VisionModelType.SIMPLE_ANALYSIS:
                    ...
                    logger.warning(f"[MultiVisionAnalyzer] {config.name} 未启用 (缺少API密钥)")
            ...
            logger.info(f"[MultiVisionAnalyzer] 初始化完成，已启用 {len(available_models)} 个模型")
    ...
    async def analyze_image(self, image_data: bytes, max_retries: int = 3) -> ImageAnalysisResult:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"[MultiVisionAnalyzer] 使用 {selected_model.name} 分析图片 (尝试 {attempt + 1}/{max_retries})")
    ...
    async def _call_vision_api(self, model_config: VisionModelConfig, 
                              image_base64: str, image_format: str) -> Dict[str, Any]:
        """调用视觉模型API"""
