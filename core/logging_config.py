...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.levelno >= logging.ERROR:
            ...
            if record.exc_info:
                fmt += f"\n{color}Exception:{reset} %(exc_info)s"
        else:
            fmt = f"{color}[%(asctime)s] [%(levelname)s] %(message)s{reset}"
...
def setup_logging(config: Optional[LogConfig] = None) -> None:
    ...
    if config.console_enabled:
        ...
        log_config_dict['handlers']['console'] = {
            ...
            'level': config.level.value,
            'formatter': 'color' if sys.stderr.isatty() and config.format != LogFormat.JSON else config.format.value,
            'stream': sys.stderr
        }
