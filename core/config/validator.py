...
class AIConfig(BaseModel):
    ...
    def validate_provider(cls, v):
        """验证提供商"""
        valid_providers = ['openai', 'claude', 'deepseek', 'zhipuai', 'mock']
        if v not in valid_providers:
            raise ValueError(f"provider必须是以下之一: {valid_providers}")
        return v
...
def get_default_config() -> AppConfig:
    ...
    return AppConfig(
        ai=AIConfig(
            provider="openai",
            api_key="your-api-key-here",  # 用户需要替换
            model="gpt-3.5-turbo"
        )
    )
