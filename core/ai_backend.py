...
class AIBackend:
    ...
    def _init_provider(self):
        ...
        if self.provider == AIProvider.DEEPSEEK:
            self.base_url = self.base_url or get_api_url("deepseek")
            self.model = self.model or "deepseek-chat"

        elif self.provider == AIProvider.OPENAI:
            self.base_url = self.base_url or get_api_url("openai")
            self.model = self.model or "gpt-4"

        elif self.provider == AIProvider.ANTHROPIC:
            self.base_url = self.base_url or get_api_url("anthropic")
            self.model = self.model or "claude-3-sonnet-20240229"

        elif self.provider == AIProvider.SILICONFLOW:
            self.base_url = self.base_url or get_api_url("siliconflow")
...
{{
    ...
    "parallel_commands": {{
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一个智能任务分析助手，帮助用户规划和执行终端任务。",
            },
    ...
    async def plan_collaborative_task(self, task: str) -> Dict:
        """规划协同任务
...
{{
    ...
    async def analyze_terminal_status(self, status_data: Dict) -> Dict:
        """分析终端状态
...
请提供简洁的分析报告。"""
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一个系统分析专家，擅长终端和系统状态分析。",
            },
            {"role": "user", "content": prompt},
        ]
    ...
    async def generate_fix(self, error: str, context: Dict) -> Dict:
        """生成修复方案
...
请提供实用的修复方案。"""
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一个问题解决专家，擅长诊断和修复技术问题。",
            },
            {"role": "user", "content": prompt},
        ]
    ...
    async def suggest_next_action(
        self, current_task: str, result: Dict, context: Dict
    ) -> Dict:
        """建议下一步操作
