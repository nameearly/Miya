                            ...
                            for line in f:
                                if line.startswith("nameserver"):
                                    ...
                                    if ip.startswith(("10.", "172.", "192.168.")):
                                        print(
                                            f"[终端代理] 检测到WSL环境，使用Windows主机IP: {ip}"
                                        )
                                        return ip
...
class MiyaTerminalAgent:
    ...
    def __init__(self, session_id: str, host: str = None, port: int = None):
        ...
        if host is None:
            ...
            self.host = host
        self.port = port or get_constant("terminal", "api_port", 8000)
        self.running = True
    ...
    async def connect_to_miya(self) -> bool:
        ...
        ports_to_try = get_constant("terminal", "api_ports", [8000, 8080, 8001, 8888])
    ...
    def _should_stream_output(self, text: str) -> bool:
        ...
        if chinese_count > 10 or any(
            indicator in text for indicator in chat_indicators
        ):
            return True
        ...
        for attempt in range(max_retries):
            try:
                    ...
                    if resp.status == 200:
                        ...
                        if data.get("status") == "error":
                            ...
                            if attempt < max_retries - 1:
                                print(
                                    f"[终端代理] 重试中... ({attempt + 1}/{max_retries})"
                                )
                        ...
                        error_text = (
                            await resp.text()
                            if resp.content
                            else f"状态码: {resp.status}"
                        )
                        print(
                            f"[终端代理] 请求失败 (状态码: {resp.status}): {error_text[:200]}"
                        )
                ...
                if attempt < max_retries - 1:
                    ...
                    return get_error_message("system_error")
    ...
    async def notify_session_end(self) -> bool:
        ...
        try:
            ...
            url = f"http://{self.host}:{self.port}/api/chat/session_end"
...
async def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="interactive", choices=["interactive", "command"]
    )
