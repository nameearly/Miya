...
PLATFORM_TOOL_MAP = {
    'qq': [
        'send_message', 'get_user_info', 'qq_like', 'send_poke',
        'react_emoji', 'get_member_list', 'get_member_info', 'find_member',
        'memory_add', 'memory_list',
        'execute_on_desktop', 'send_to_desktop', 'send_to_terminal',
        'terminal_command',
        'knowledge_text_search', 'knowledge_semantic_search',
        'start_trpg', 'roll_dice', 'search_tavern_characters',
    ],
    'terminal': [
        'terminal_command', 'multi_terminal', 'wsl_manager',
        'system_info', 'environment_detector',
        'send_to_qq', 'send_to_desktop', 'qq_like',
    ],
    'desktop': [
        'execute_on_desktop', 'send_to_desktop',
        'send_to_qq', 'send_to_terminal',
        'sync_state', 'terminal_command',
    ],
    'web': [
        'send_to_qq', 'send_to_desktop',
...
class ResponseGenerator:
    ...
    def _get_platform_specific_tools(self, platform: str) -> list:
        ...
        try:
            ...
            logger.info(f"[响应生成器] 平台 {platform} 使用 {len(platform_schemas)} 个工具")
