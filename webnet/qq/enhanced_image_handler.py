...
class EnhancedQQImageHandler:
    ...
    def __init__(self, qq_net):
        ...
        if self.multi_model_enabled:
            ...
            logger.warning(
                "[EnhancedQQImageHandler] 初始化完成 - 多模型系统不可用，将使用简化模式"
            )
    ...
    async def initialize(self):
        ...
        if self.multi_model_enabled and self.vision_analyzer is None:
            try:
                ...
                logger.info(
                    f"[EnhancedQQImageHandler] 多模型系统已初始化，{stats['enabled_models']} 个模型可用"
                )
    ...
    def _init_auto_saver(self):
        ...
        if AUTO_SAVER_AVAILABLE:
            try:
                ...
                logger.error(
                    f"[EnhancedQQImageHandler] 初始化表情包自动保存服务失败: {e}"
                )
                self.auto_saver = None
        else:
    ...
    def configure(self, enable_ocr: bool = False, enable_ai_analysis: bool = True):
        ...
        if self.multi_model_enabled:
            logger.info(
                "[EnhancedQQImageHandler] 配置完成 - 使用多模型分析系统（自动故障转移）"
            )
        else:
            logger.warning(
                "[EnhancedQQImageHandler] 配置完成 - 使用简化模式（无多模型支持）"
            )
    ...
    async def handle_image_message(self, event: Dict) -> Optional[QQMessage]:
        ...
        try:
            ...
            if analysis_result.success:
                ...
                logger.error(
                    f"[EnhancedQQImageHandler] 图片分析失败: {analysis_result.error_message}"
                )
    ...
    async def _try_auto_save_user_emoji(
        self, event: Dict, image_data: bytes, image_info: Dict
    ) -> None:
        """
        尝试自动保存用户发送的表情包
        ...
        try:
            ...
            logger.info(
                f"[EnhancedQQImageHandler] 尝试自动保存用户 {sender_id} 发送的表情包"
            )

            # 调用自动保存服务
            save_result = await self.auto_saver.auto_save_emoji(
                user_id=sender_id, image_data=image_data, image_info=image_info
            )

            if save_result["success"]:
                logger.info(
                    f"[EnhancedQQImageHandler] 表情包自动保存成功: {save_result.get('filename', 'unknown')}"
                )
                ...
                if notifications_config.get("on_save_success", True):
                    ...
                    logger.debug(
                        f"[EnhancedQQImageHandler] 保存成功消息: {save_result['message']}"
                    )
            else:
                logger.info(
                    f"[EnhancedQQImageHandler] 表情包自动保存跳过: {save_result['message']}"
                )
                ...
                if notifications_config.get("on_save_fail", False):
                    logger.debug(
                        f"[EnhancedQQImageHandler] 保存失败消息: {save_result['message']}"
                    )

        except Exception as e:
            logger.error(
                f"[EnhancedQQImageHandler] 自动保存表情包异常: {e}", exc_info=True
            )
            # 不抛出异常，避免影响正常的图片处理流程
    ...
    async def _download_image(self, image_info: Dict) -> Optional[bytes]:
        ...
        try:
            ...
            if cache_key in self.image_cache:
                ...
                ).total_seconds() < self.cache_expire_hours * 3600:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"[EnhancedQQImageHandler] 下载图片失败: HTTP {response.status_code}"
                )
                return None
    ...
    def _cache_image(self, image_id: str, image_data: bytes):
        """缓存图片"""
        self.image_cache[image_id] = {"data": image_data, "timestamp": datetime.now()}
    ...
    def _cleanup_cache(self):
        ...
        if expired_keys:
            logger.info(
                f"[EnhancedQQImageHandler] 清理了 {len(expired_keys)} 个过期缓存"
            )

    def _create_response_message(
        self, event: Dict, analysis_result: ImageAnalysisResult
    ) -> QQMessage:
        """创建回复消息（弥娅风格）"""
        from .models import QQMessage
        ...
        if analysis_result.model_used != "简单分析":
            ...
            if analysis_result.processing_time_ms > 0:
                response_text += (
                    f"（花了{analysis_result.processing_time_ms:.0f}毫秒来分析呢~）"
                )
        ...
        # 创建消息对象
        group_id = (
            event.get("group_id", 0) if event.get("message_type") == "group" else 0
        )
