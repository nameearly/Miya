...
class Settings:
    ...
    def _merge_qq_configs(self, base_config: Dict, yaml_config: Dict) -> Dict:
        ...
        merged = {
            ...
            # 3. 访问控制（从.env解析）
            'access_control': {
                'group_whitelist': self._parse_qq_list(base_config.get('group_whitelist', '')),
                'group_blacklist': self._parse_qq_list(base_config.get('group_blacklist', '')),
                'user_whitelist': self._parse_qq_list(base_config.get('user_whitelist', '')),
                'user_blacklist': self._parse_qq_list(base_config.get('user_blacklist', '')),
                'enabled': False,  # 默认禁用，除非配置了列表
            },
        }
        
        # 4. 添加YAML中的详细配置
        for section in ['multimedia', 'image_recognition', 'active_chat', 
                       'task_scheduler', 'performance', 'logging', 'debug']:
        ...
        if 'image_recognition' in merged:
            ...
            merged['image_recognition']['ocr']['enabled'] = base_config.get('ocr_enabled', True)
            merged['image_recognition']['ocr']['engine'] = base_config.get('ocr_engine', 'auto')
        
        if 'active_chat' in merged:
            merged['active_chat']['enabled'] = base_config.get('active_chat_enabled', True)
        
        if 'task_scheduler' in merged:
            merged['task_scheduler']['enabled'] = base_config.get('task_scheduler_enabled', True)
        ...
        return {
            ...
            'multimedia': {
                'image': {
                    ...
                    'allowed_formats': os.getenv('QQ_IMAGE_ALLOWED_FORMATS', '.jpg,.jpeg,.png,.gif,.bmp,.webp').split(','),
                },
                'file': {
